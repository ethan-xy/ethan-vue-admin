<template>
  <el-card>
    ...
    <permission-group-form-dialog action="add" @handleRow="onHandleRow" v-model="addDialogVisible"></permission-group-form-dialog>
    <permission-group-form-dialog action="edit" @handleRow="onHandleRow" :row="nowRow" :rowIndex="rowIndex" v-model="editDialogVisible"></permission-group-form-dialog>
  </el-card>
</template>
<script>
  import {computed, ref} from 'vue'
  import { useStore } from 'vuex'
  import {getPermissionGroup, deletePermissionGroup} from "@/api/permission-group"
  import {tableDataDefault, tableDataFormat} from '@/utils/table'
  import PermissionGroupFormDialog from './PermissionGroupFormDialog.vue'
  ...
  export default {
    ...
    setup() {
      ...
      const requestData = () => {
        ...
        getPermissionGroup(tableData.getQueryParams()).then(res => {
      ...
      return {
        ...
        handleDelete,
        updatePermission: computed(() => store.getters.hasPermission('permission-group.update')),
        storePermission: computed(() => store.getters.hasPermission('permission-group.store')),
        deletePermission: computed(() => store.getters.hasPermission('permission-group.destroy')),
      }
    }
  }
</script>
